...
window.updateComparisonUI = function(ids) {
    ...
    const fmt = (m, key) => {
        let v = m.stats[key];
        if (key === 'hardness') v = v + (m.stats.unit || '');
        return v;
    };
    const rows = [
    ...
    const rowsHtml = rows.map(r => `<tr class="border-b border-slate-100">
        ...
        <td class="py-1.5 px-1 text-center font-bold ${a.text}">${fmt(a, r.key)}</td>
        <td class="py-1.5 px-1 text-center font-bold ${b.text}">${fmt(b, r.key)}</td>
    </tr>`).join('');
